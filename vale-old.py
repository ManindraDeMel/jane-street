...
def update_positions_after_buy(symbol, quantity):
    global positions
    positions[symbol] += quantity

def update_positions_after_sell(symbol, quantity):
    global positions
    positions[symbol] -= quantity

def update_positions_for_conversion(buy_symbol, sell_symbol, quantity):
    global positions
    # Assuming a 1:1 conversion rate for simplicity; adjust based on actual conversion rates
    positions[buy_symbol] += quantity
    positions[sell_symbol] -= quantity
...
def execute_conversion_strategy(exchange, order_id):
    ...
    if best_ask_prices.get('VALE', float('inf')) < best_bid_prices.get('VALBZ', 0):
        ...
        if (positions['VALE'] != 9) and (positions['VALBZ'] != -9):
            exchange.send_add_message(order_id, "VALE", Dir.BUY, best_ask_prices['VALE'], 5)
            order_id += 1
            message_sent = True  # Set flag to True after sending a message
            update_positions_after_buy("VALE", 5)

        # Convert 20 VALE to VALBZ
        # if (positions['VALE'] != 9) and (positions['VALBZ'] != -9):
        #     exchange.send_convert_message(order_id, "VALE", Dir.SELL, 5)
        #     order_id += 1
        #     message_sent = True
        #     update_positions_for_conversion("VALBZ", "VALE", 5)
        

        
        # Sell 20 VALBZ
        if (positions['VALE'] != 9) and (positions['VALBZ'] != -9):
            exchange.send_add_message(order_id, "VALBZ", Dir.SELL, best_bid_prices['VALBZ'], 5)
            order_id += 1
            message_sent = True
            update_positions_after_sell("VALBZ", 5)
        ...
        print(f"Condition for Vale {best_ask_prices.get('VALE', float('inf'))} -> ValeBZ not met {best_bid_prices.get('VALBZ', 0)}")
    
    
    if best_ask_prices.get('VALBZ', float('inf')) < best_bid_prices.get('VALE', 0):
        # Buy 20 VALE
        if (positions['VALE'] != -9) and (positions('VALBZ')  != 9):
            exchange.send_add_message(order_id, "VALBZ", Dir.BUY, best_ask_prices['VALBZ'], 5)
            order_id += 1
            message_sent = True  # Set flag to True after sending a message
            update_positions_after_buy("VALE", 5)
        ...
        # Convert 20 VALE to VALBZ
        # if (positions['VALE'] in range(-5, 5)) and (positions('VALBZ') in range(-5, 5)):
        #     exchange.send_convert_message(order_id, "VALE", Dir.BUY, 5)
        #     order_id += 1
        #     message_sent = True
        #     update_positions_for_conversion("VALE", "VALBZ", 5)

        # Wait for conversion confirmation (not shown here)
        
        # Sell 20 VALBZ
        if (positions['VALE'] in range(-5, 5)) and (positions('VALBZ') in range(-5, 5)):
            exchange.send_add_message(order_id, "VALE", Dir.SELL, best_bid_prices['VALE'], 5)
            order_id += 1
            message_sent = True
            update_positions_after_sell("VALE", 5)
        ...
        print(f"Condition for ValBZ {best_bid_prices.get('VALBZ', 0)} -> Vale not met {best_ask_prices.get('VALE', float('inf'))}")
